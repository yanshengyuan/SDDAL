...
class M290(nn.Module):
    def __init__(self, batchsize, beamshape, path_lightsource, device, imaging_plane='prefoc'):
        ...
        self.raw_zernike = nn.Parameter(torch.zeros((batchsize, 12, 1, 1), dtype=torch.float32))
    
    
    def get_zernike_coeffs(self, bound=1.5):
        return bound * torch.tanh(self.raw_zernike)
    
        
    def forward(self, field):
        
        zernike_coeffs = self.get_zernike_coeffs()
        
        phase = torch.sum(zernike_coeffs * self.Zernike_modes, dim=1)
