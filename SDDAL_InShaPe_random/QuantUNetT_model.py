...
class QuantUNetT(nn.Module):
    def __init__(self, block=ConvBlock,dim=32):
        super(QuantUNetT, self).__init__()
    ...
    def forward(self, x):
        ...
        up6 = self.upv6(conv5)
        up6 = pad_to_match(up6, conv4)
        up6 = torch.cat([up6, conv4], 1)
        conv6 = self.ConvBlock6(up6)

        up7 = self.upv7(conv6)
        up7 = pad_to_match(up7, conv3)
        up7 = torch.cat([up7, conv3], 1)
        conv7 = self.ConvBlock7(up7)

        up8 = self.upv8(conv7)
        up8 = pad_to_match(up8, conv2)
        up8 = torch.cat([up8, conv2], 1)
        conv8 = self.ConvBlock8(up8)

        up9 = self.upv9(conv8)
        up9 = pad_to_match(up9, conv1)
